  ...
  const ratedMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isRated) {
        ...
        const { error } = await supabase.from("rated_content").insert({
          user_id: user.id,
          content_id: movie.id,
          rating: rating,
        });
          ...
          <div className="flex flex-col gap-2">
            <Button
              ...
              className={`w-full flex items-center justify-start gap-2 ${
                isInWatchlist ? "bg-primary/10 border-primary/20" : ""
              }`}
              onClick={handleWatchlistClick}
            >
              <List
                className={`h-4 w-4 ${isInWatchlist ? "text-primary" : ""}`}
              />
            ...
            <Button
              variant="outline"
              className={`w-full flex items-center justify-start gap-2 ${
                isRated ? "bg-primary/10 border-primary/20" : ""
              }`}
              onClick={handleRateClick}
            >
              <Star
                className={`h-4 w-4 ${
                  isRated ? "text-yellow-500 fill-yellow-500" : ""
                }`}
              />
